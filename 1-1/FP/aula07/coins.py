...
def transfer1coin(bag1, c, bag2):
    ver = False    
    if (bag1.get(c) != 0 and bag1.get(c) != None):
        ver = True
        bag1[c] = bag1[c]-1
        if (bag2.get(c) != None):
            bag2[c] += 1
...
def transfer(bag1, amount, bag2):
    ...
    sortedbag1 = sorted(bag1.items(), key=lambda x:x[0], reverse=True)
    if amount == 0:
        return True
    if value(bag1) < amount:
        return False
    else:
        for i in range(0, len(sortedbag1)):
            tempammount = 0
            templistTransaction = []
            if sortedbag1[i][0] > amount:
                continue
            else:
                e = i
                numberofCoins = 0
                while (tempammount <= amount and e < len(sortedbag1)):
                    if ((tempammount+sortedbag1[e][0]) > amount):
                        templistTransaction.append((sortedbag1[e][0], numberofCoins))
                        e += 1
                        numberofCoins = 0
                    elif numberofCoins >= sortedbag1[e][1]:
                        e += 1
                    else:
                        tempammount += sortedbag1[e][0]
                        numberofCoins += 1
            if tempammount == amount:
                for i in templistTransaction:
                    if i[1] != 0:
                        bag1[i[0]] -= i[1]
                        if (bag2.get(i[0]) != None):
                           bag2[i[0]] += i[1]
                        else:
                            bag2[i[0]] = i[1]      
                return True
...
def strbag(bag):
    ...
    if len(bag) != 0:
        tempmessage = ""
        value = 0
        for i in sorted(bag.items(), key=lambda x:x[0], reverse=True):
            value += i[0] * i[1]
            if i[1] != 0:
                tempmessage += "{}x{}+".format(i[1],i[0])
        tempmessage = tempmessage.rstrip(tempmessage[-1])
        tempmessage += ("={}".format(value))
        bag = tempmessage
