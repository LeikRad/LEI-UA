...
def main():
    ...
    tab = sorted(tabela, key=lambda x: pontos(x), reverse=True)
    printTabela(tab)

    # c)
    # Acrescente os argumentos adequados à função sorted para
    # obter uma tabela ordenada por ordem decrescente da diferença GM-GS:
    tab = sorted(tabela, key=lambda x: x[GM]-x[GS], reverse=True)
    printTabela(tab)
    ...
    tab = sorted(tabela, key=lambda x: (pontos(x), x[GM]-x[GS]), reverse=True)
    printTabela(tab)
